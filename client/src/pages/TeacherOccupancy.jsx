    ...
    try {
      ...
      const resolvedSchedules = await Promise.all(
        teacherSchedules.map(async (schedule) => {
          ...
          const cKey = String(schedule.courseId || schedule.course || "").trim();
          if (cKey) {
            const courseMap = await fetchCoursesCache();
            let courseObj = courseMap.get(cKey);
            if (!courseObj && courseMap) {
              for (const [_, c] of courseMap) {
                if (
                  String(c.unid) === cKey ||
                  String(c.ID) === cKey ||
                  String(c.code) === cKey ||
            ...
            if (courseObj) {
              resolved.code = courseObj.code || courseObj.ID || "";
              resolved.course = courseObj.name || courseObj.ID || "";
              resolved.batch = courseObj.batchName || schedule.batch || "";
            } else if (schedule.course && !/^\d{10,}$/.test(String(schedule.course).trim())) {
              resolved.code = schedule.code || schedule.course;
              resolved.course = schedule.course;
            } else {
              resolved.course = await getCourseDisplayName(cKey);
            }
          }
